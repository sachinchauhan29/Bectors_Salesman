...
class Welcome extends CI_Controller
{
	...
	public function cardthanks()
	{
		...
		$data['user_data'] = $this->db->get_where('scratch_code', array('number' => $MobileNo, 'active_date' => $date))->result_array();
		// echo '<pre>';
		// print_r($data);
		// die;
	...
	public function thankyou()
	{
		...
		while ($x <= $card) {
			...
			$arr1 = array('Name' => $data['Name'], 'code' => $rrcode, 'Market' => $data['state'], 'Beatname' => $data['beat_name'], 'number' => $data['MobileNo'], 'Vanaspati' => $rtmr, 'scode' => $scode, 'amount' => $rs['amount'], 'active_date' => date("Y-m-d"), 'time' => $time);
			// print_r($arr1);
			// die;
